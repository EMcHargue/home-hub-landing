...
const httpTrigger: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
  ...
  try {
    ...
    if (method === "GET") {
      const result = await pool.request().query("SELECT * FROM dbo.chores ORDER BY created_at DESC");
      context.res = { status: 200, body: result.recordset };

    } else if (method === "POST") {
      const { title, description, assignee_id, due_date, recurrence } = req.body;
      if (!title) {
      ...
      const result = await pool
        ...
        .input("recurrence",  sql.NVarChar(20),  recurrence || "none")
        .query(`INSERT INTO dbo.chores (title, description, assignee_id, due_date, recurrence)
                OUTPUT INSERTED.*
                VALUES (@title, @description, @assignee_id, @due_date, @recurrence)`);
      context.res = { status: 201, body: result.recordset[0] };

    } else if (method === "PUT" && id) {
      const { title, description, assignee_id, due_date, recurrence, completed } = req.body;
      ...
      if (recurrence !== undefined) { request.input("recurrence", sql.NVarChar(20), recurrence); setClauses.push("recurrence = @recurrence"); }
      ...
      await request.query(`UPDATE dbo.chores SET ${setClauses.join(", ")} WHERE id = @id`);
      context.res = { status: 200, body: { success: true } };

    } else if (method === "DELETE" && id) {
      await pool.request().input("id", sql.UniqueIdentifier, id).query("DELETE FROM dbo.chores WHERE id = @id");
      context.res = { status: 200, body: { success: true } };
